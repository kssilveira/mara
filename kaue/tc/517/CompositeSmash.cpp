...
void get(int target, int& target_div, int& target_exp) {
    target_div = 0;
    target_exp = 0;
    for (int i = 2; i <= sqrt(target); ++i) {
      if (target % i == 0) {
        target_div++;
        if (target_div > 1) break;
      }
      while (target % i == 0) {
        target /= i;
        target_exp++;
...
class CompositeSmash {
    ...
    int target_div, target_exp, n_div, n_exp;
    get(target, target_div, target_exp);
    get(N, n_div, n_exp);

    // printf("%i %i %i %i\n", target_div, target_exp, n_div, n_exp);

    if (target_div == 1 && target_exp == 1) return "Yes";
    if (n_div > 1 || target_div > 1) return "No";
    if (target_exp >= 3) return "No";
